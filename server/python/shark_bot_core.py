...
class InstitucionalSMCBot:
    def __init__(self, dataframe, balance=10000.0, risk_per_trade=0.01):
        """
        Inicializa o robô com o histórico de preços e configurações de risco.
        balance: Capital total da conta (ex: $10.000)
        risk_per_trade: Risco máximo por operação (1% = 0.01)
        """
    ...
    def _calcular_atr(self, period=14):
        """Calcula o Average True Range (Filtro de Volatilidade)"""
    ...
    def _mapear_estrutura_mercado(self):
        """Mapeia os Topos/Fundos, Quebra de Estrutura (BOS) e Zona de Desconto"""
    ...
    def _detectar_fvg_alta(self):
        """Encontra e valida matematicamente os Fair Value Gaps"""
        self.df['Gap_Size'] = self.df['Low'].shift(2) - self.df['High']
        
        fvg_existe = self.df['Low'].shift(2) > self.df['High']
        
        fvg_relevante = self.df['Gap_Size'] > (0.5 * self.df['ATR'])
    ...
    def gerar_sinais(self):
        """Roda a arquitetura e retorna apenas os momentos exatos de entrada"""
        self._calcular_atr()
    ...
    def calcular_lote(self, preco_entrada, preco_stop):
        """Garante que a perda será exatamente 1% da conta, independente do tamanho do Stop"""
